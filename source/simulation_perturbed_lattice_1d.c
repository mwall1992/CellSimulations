	...
	for (int i = 0; i < kNumBounds; i++) {
		...
		if (performSimulation) {
			...
			*agentId = 1;
			...
			sprintf(trackedAgentFileName, "tracked_agent_pos_%0.02f.txt", bounds[i]);
			bool isTracked =  tracked_agents_parser(trackedPositions, 
				timeSteps + 1, 0, trackedAgentFileName, "output/");
			if (isTracked) {
				printf("Stored tracking information (delta: %0.02f).\n", bounds[i]);
			} else {
				printf("Error: failed storing tracking information (delta: %0.02f).\n", 
					bounds[i]);
			}
