...
int main(int argc, char* argv[]) {
	...
	if (saveNodeLocations) {
		sprintf(latticeLayoutFileName, "node_positions_single_%d.txt", 
			columns);
		...
		if (trackedLatticeLayout) {
			printf("Stored lattice layout information.\n");
		} else {
			printf("Error: failed storing lattice layout information.\n");
		}
	}
	...
	for (int i = 0; i < kSimulationCount; i++) {
		// populate lattice
